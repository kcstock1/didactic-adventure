...
async function assign_load(boat_id,load_id,req,res){
   ...
   try{ var lkey = datastore.key([LOAD, parseInt(load_id,10)]);  
    ...
    res.status(200).send(JSON.stringify("Load added to boat"));
    //return boat[0]; 
 ...
 function delete_boat(id,req,res){ 
     const key = datastore.key([BOAT, parseInt(id,10)])
     get_boat(id,req)
     .then(result => {
         result[0].loads.forEach( 
             element => {
                 return get_load(element.id)
             .then(element => {
     ...
     .then(results => {return datastore.delete(key)}  
         ) 
     .catch(err => {
...
function delete_load(id,req,res){
    
    const key = datastore.key([LOAD, parseInt(id,10)]);
    var load = get_load(id,req)
    .then(results =>{
...
router.delete('/boats/:id', function(req, res){
        ...
        var Error = {"Error":"The request object is missing at least one of the required attributes"}; 
        delete_boat(req.params.id,req,res).then(results => {
